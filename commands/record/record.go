...
func Handler(pathWD string, args []string, isBuilt bool, importPrefix string) (err error) {
	...
	case verbAdd:
		if !isBuilt {
			fmt.Println("The application must be initialized before a record can be added.")
			return
		}
		if len(args) < 2 {
			fmt.Println(Usage())
	...
	case verbRemove:
		if !isBuilt {
			fmt.Println("The application must be initialized before a record can be removed.")
			return
		}
		if len(args) < 2 {
			fmt.Println(Usage())
