...
1. Creates it's application framework code in a folder that contains it's go.mod file.
1. Adds and removes screens and their panels in the framework's front-end GUI.
...
### This is version 0.9.1. It brings improvements to

* Improvements to documentation.
* A simpler and better front end.
* Various bug fixes or code improvements.
...
* Documentation edits.
* Faster startup.
* Can use custom application meta data with the file **FyneApp.toml**.
* The data stores auto close on shut down after all transactions are completed.
* Reduced framework code.

## Framework summary

All of the applicatioinlication code is contained in 4 folders.
...
### The front-end's GUI has some of what you would expect in a fyne application

1. An optional application icon.
1. A system tray menu
1. An application menu.
1. A landing screen.
1. Any other screens a developer wants to add with kickfyne.
1. At startup, the application window opens almost immediately and before the panels in the screens might have completed filling with dynamic content.
...
1. The selectPanel:
   ...
   * The select button passes the selection info in a call to a messenger send func.
   * The messenger sends the selection info to the back-end in a message.
   * When the back-end returns the message it contains the info for the user to edit. The messenger receives the message in a recive func and passes the info in a call to an editPanel func.
...
1. The editPanel:
   ...
   * The edit button passes the user input in a call to a messenger send func.
   ...
   * The messenger receives tge message in a receive func and informs the user of the success or failure using fyne's dialog package.
   * The messenger switches the user back to the landing screen with the call **landingscreen.Land()**.

#### The landing screen

The applicatioinlication begins with the landing screen. Like any other screen, the landing screen operation needs to be developed and managed by the developer.

The applicatioinlicationframework creates a **home** screen at **frontend/gui/screens/home/** as the landing screen. The developer can use the home screen or add a new screen and then change the landing screen in the ./FyneApp.toml file. After the ./FyneApp.toml file is edited, the developer can update the applicatioinlicationto the new screen and remove the home screen.
...
BUILDING THE APP:
You can build the applicatioinlicationafter running the command `＄ kickfyne framework`.
...
1. The following success message is displayed. The file paths displayed, link to the files that were just added and need attention. A search for KICKFYNE TODO will reveal instructions for proper developement and management of the message and handler operation.
   ...
   KICKFYNE TODO: Some front-end screen message handlers will need to use the message.
   ```
...
1. The following success message is displayed. The file paths displayed, link to the files that were just added and need attention. A search for KICKFYNE TODO will reveal instructions for proper developement and management of the record and store operation.
   ...
   KICKFYNE TODO: The record definition at shared/record/contact.go:1:1 may need some editing.
   KICKFYNE TODO: The storer interface definition at backend/store/storer/contact.go:1:1 may need some editing.
   KICKFYNE TODO: The storer interface implementation at backend/store/storing/contact.go:1:1 may need some editing.
   ```

1. The record's store interface and implementation may need modification. The records are by default, stored locally in easy to read yaml files. If that is not how the applicatioinlication is to handle data then modifications can be made to a record's store interface and implementation.
